...
void main(void) {	
	vec3 position_EC = vec3(u_ModelViewMatrix*vec4(a_position, 1.0f));
	vec3 normal_EC = normalize(u_ModelViewMatrixInvTrans*a_normal);  
	vec2 v_tex_coord = a_tex_coord;
	...
	shaded_color = lighting_equation_textured(position_EC, normal_EC, base_color);

	v_color = shaded_color;
}