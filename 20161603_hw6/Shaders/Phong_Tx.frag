...
uniform int u_number_of_tiles = 1;
uniform bool u_fun = false;
uniform float u_spotlight_modifier;
uniform bool u_screen = false;
			...
			if (u_light[i].spot_cutoff_angle < 180.0f) { // [0.0f, 90.0f] or 180.0f
				...
				if (tmp_float >= cos(radians(spot_cutoff_angle))) {
					if(!u_fun)
						...
						tmp_float = cos(u_spotlight_modifier*acos(tmp_float));
						if (tmp_float < zero_f)
							tmp_float = zero_f;
					}
				}
...
// For a normal rendering ...
	...
	if (u_flag_texture_mapping) 
		base_color = texture(u_base_texture, mod(v_tex_coord*u_number_of_tiles, 1.0f));
	else 
...
//      fog_factor = (FOG_FAR_DISTANCE + v_position_EC.z)/(FOG_FAR_DISTANCE - FOG_NEAR_DISTANCE);
	...
	if(u_screen) {
		vec2 coord = mod(v_tex_coord * u_number_of_tiles, 1.0f);
		if(coord.x + coord.y <= 0.5f || coord.x + coord.y >= 1.5f) {
			discard;
		}
	}
#endif
